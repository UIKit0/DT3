...
//==============================================================================
		...
		#define ASSERT(cond)    if (!(cond))        DT3::AssertImpl::call_assert_msg(__FILE__, __FUNCTION__, __LINE__)
	#else
		#define ASSERT(cond)    if (!(cond))        DT3::AssertImpl::call_assert_msg(__FILE__, __func__, __LINE__)
	#endif

	#if DT3_OS == DT3_WINDOWS
		#define ASSERTBREAK(cond)   if (!(cond))    DT3::AssertImpl::call_assert_break(__FILE__, __FUNCTION__, __LINE__)
	#else
		#define ASSERTBREAK(cond)   if (!(cond))    DT3::AssertImpl::call_assert_break(__FILE__, __func__, __LINE__)
	#endif
