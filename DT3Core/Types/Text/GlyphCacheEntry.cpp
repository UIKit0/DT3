...
{   
    ...
    if (!_glyph) {
        ...
        FT_Matrix matrix;
        matrix.xx = static_cast<FT_Fixed>( _transform.x_axis().x * 0x10000L );
        matrix.yx = static_cast<FT_Fixed>( _transform.x_axis().y * 0x10000L );
        matrix.xy = static_cast<FT_Fixed>( _transform.y_axis().x * 0x10000L );
        matrix.yy = static_cast<FT_Fixed>( _transform.y_axis().y * 0x10000L );
