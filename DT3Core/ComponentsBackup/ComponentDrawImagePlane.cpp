...
void ComponentDrawImagePlane::archive_read (const std::shared_ptr<Archive> &archive)
{
...
void ComponentDrawImagePlane::archive_write (const std::shared_ptr<Archive> &archive)
{
...
void ComponentDrawImagePlane::draw (const std::shared_ptr<CameraObject> &camera, const DTfloat lag)
{
    ...
    std::shared_ptr<PlaceableObject> placeable = checked_cast<PlaceableObject>(owner());
    if (!placeable)
        return;
    ...
    b.batch_begin(  camera,
                    ...
                    DrawBatcher::FMT_V | DrawBatcher::FMT_T1 | DrawBatcher::FMT_C,
                    4);
    
    b.add().v(-width,+height,0.0F)  .t0(tc_s_min,tc_t_max)  .c(c);
    b.add().v(-width,-height,0.0F)  .t0(tc_s_min,tc_t_min)  .c(c);
    b.add().v(+width,+height,0.0F)  .t0(tc_s_max,tc_t_max)  .c(c);
    b.add().v(+width,-height,0.0F)  .t0(tc_s_max,tc_t_min)  .c(c);
...
void ComponentDrawImagePlane::add_to_owner (const std::shared_ptr<ObjectBase> &owner)
{
    ComponentBase::add_to_owner(owner);
    
    std::shared_ptr<World> w = owner->world();

    w->register_for_draw(owner.get(), make_callback(this, &type::draw));
}
...
void ComponentDrawImagePlane::remove_from_owner (void)
{
    ...
    w->unregister_for_draw(owner().get(), make_callback(this, &type::draw));
